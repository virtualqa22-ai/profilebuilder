...
app.use('/api/resumes', resilientHandler(async (req, res, next) => {
  // Wrap the route handler
  const originalSend = res.send;
  res.send = function(data) {
    originalSend.call(this, data);
  };
  return resumeRoutes(req, res, next);
}));
...
app.get('/health', (req, res) => {
  ...
  res.json({
    ...
    service: 'resume-management-service',
...
connectDB().then(() => {
  app.listen(PORT, () => {
    logger.info(`Resume Management Service running on port ${PORT} with resilience patterns enabled`);
  });
}).catch((error) => {
